...
struct C_CS_PlayerPawn {
	...
	std::string GetWeapon() {
		...
		uintptr_t weapon_adr = interfaces::mem.Read<uintptr_t>(p_entity + offsets::m_designerName);
		std::string weapon = interfaces::mem.ReadString(weapon_adr, 256);
		std::string rem = "weapon_";
		size_t pos = weapon.find(rem);
		if(pos != std::string::npos) {
			// Erase the substring from the original string
			weapon.erase(pos, rem.length());
		}
		return weapon;
	}
